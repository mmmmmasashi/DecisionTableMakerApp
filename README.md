...
- **足し算 (`+`)**: 因子間の一部の組み合わせを生成します。
  ...
  - 実施しやすいように、水準は順番に並べられます。
- **溶かしこみ(`<`)**: 因子の水準を組み合わせて新たな因子を生成します。
  - 例: `[OS] * [Language] < [Version(1,2,3,4)]` は、`OS` と `Language` の全ての組み合わせを生成し、そのケース数を極力増やさない範囲で、Versionを振ります。
  - 2因子ペアでの網羅率を極力高くするように、ランダムに振られます。
  - 水準の並び順は指定されません。
  - Point : 掛け算ほど網羅する必要はないが、足し算より2因子網羅率を上げたい場合に使用します。:
